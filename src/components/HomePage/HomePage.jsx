    ...
    useEffect(() => {
        ...
        const fetchProjectPosts = async () => {
            ...
            try {
                ...
                // And ensure the full project object is available for passing to ProjectInfo
                const formattedProjects = res.data.map(project => ({
                    ...
                    university: project.createdBy ? project.createdBy.university : 'N/A', // Corrected: Using 'university' from createdBy
                    timeAgo: moment(project.createdAt).fromNow(),
                    title: project.title,
                    description: project.description,
                    technologies: project.requiredSkills || [], // ProjectCard expects 'technologies'
                    responseCount: project.joinRequests ? project.joinRequests.length : 0,
                    avatar: project.createdBy?.avatar || null, // Assuming avatar might be nested in createdBy
                    // IMPORTANT: Pass the entire original project object (or its mapped version)
                    // so ProjectCard can forward it to ProjectInfo
                    fullProjectData: { // Renamed from 'projectData' to avoid confusion with the 'project' object itself
                        projectTitle: project.title,
                        projectDescription: project.description,
                        domain: project.domain,
                        projectType: project.projectType || 'N/A', // Assuming projectType might be on backend
                        ...
                        startDate: moment(project.startDate).format('YYYY-MM-DD'), // Format for date input
                        applicationDeadline: moment(project.applicationDeadline).format('YYYY-MM-DD'), // Format for date input
                        remoteWorkOkay: project.remoteWorkOkay || false,
                        githubRepo: project.githubRepo || '',
                        figmaLink: project.figmaLink || '',
                        demoLink: project.demoLink || '',
                        // Add author details if ProjectInfo expects them for consistency,
                        // otherwise, ProjectInfo will fetch its own author details.
                        // However, if the project is "owned" by a specific user,
                        // this ensures their details are available for display.
                        authorDetails: {
                            ...
                            year: project.createdBy ? project.createdBy.currentYear : 'N/A', // Assuming currentYear from User model
                            rating: project.createdBy ? project.createdBy.rating : 0, // Assuming rating from User model
                            projectsCompleted: project.createdBy ? project.createdBy.projectsCompleted : 0 // Assuming projectsCompleted from User model
                        }
                    }
                }));
        ...
        const fetchTeamInvitations = async () => {
            ...
            try {
                ...
                console.log("Raw Invites from Backend (before mapping):", rawInvites);
                                ...
                                {projectsLoading ? (
                                    ...
                                    projectPosts.map((project) => (
                                        <ProjectCard
                                            ...
                                            // THIS IS THE CRUCIAL CHANGE: Pass the full mapped project data
                                            // The `fullProjectData` prop name is arbitrary, but should be consistent
                                            // with what ProjectCard expects (if you renamed it there too).
